...
def upload_video():
    ...
    df = pd.read_csv("chunked_data/chunk_4.csv")
    video_df = df[~df['Thumbnail'].isnull()]
    # payloads = video_df.apply(to_payload, axis=1).to_list()

    # Create pins and log success
    success_count = 0
    # for payload in payloads:
    response = pinapp.get_upload_url(media_type='video')
    ...
    #     return f"Error while fetch media upload url: {str(e)}"
